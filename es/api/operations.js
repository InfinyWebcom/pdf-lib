...
export var drawLinesOfText = function (lines, options) {
    var translatething = options.matrix ? setTextMatrix(options.matrix[0], options.matrix[1], options.matrix[2], options.matrix[3], options.x, options.y) : rotateAndSkewTextRadiansAndTranslate(toRadians(options.rotate), toRadians(options.xSkew), toRadians(options.ySkew), options.x, options.y);
    var operators = [
        pushGraphicsState(),
        options.graphicsState && setGraphicsState(options.graphicsState),
        beginText(),
